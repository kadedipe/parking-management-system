...
// ============================================================================
// Android hdpi Mipmap Icon Generator
// ============================================================================

/**
 * Script to generate all Android hdpi mipmap app icons
 * 
 * Run: node scripts/generate-hdpi-mipmap-icons.js
 * 
 * Prerequisites:
...
// hdpi mipmap icon configurations (72x72)
const HDPI_MIPMAP_CONFIG = {
  'mipmap-hdpi': {
    regular: { size: 72, name: 'ic_launcher.png' },
    round: { size: 72, name: 'ic_launcher_round.png' },
  },
};

// Adaptive hdpi icons (Android 8.0+)
const ADAPTIVE_HDPI = {
  'mipmap-hdpi': {
    size: 162,
...
async function generateHdpiMipmapIcons() {
  console.log('🎨 Generating Android hdpi mipmap app icons...');
  console.log('================================================\n');
  ...
  // Generate hdpi mipmap icons
  await generateStandardHdpiMipmapIcons(sourceBuffer);
  await generateAdaptiveHdpiIcons(sourceBuffer);
  await generateHdpiXMLFiles();

  console.log('\n🎉 All Android hdpi mipmap icons generated successfully!');
  console.log('================================================');
}

// ============================================================================
// Generate Standard hdpi Mipmap Icons
// ============================================================================

async function generateStandardHdpiMipmapIcons(sourceBuffer) {
  console.log('📱 Generating standard hdpi mipmap icons (72x72)...');

  for (const [density, config] of Object.entries(HDPI_MIPMAP_CONFIG)) {
...
// ============================================================================
// Generate Adaptive hdpi Icons
// ============================================================================

async function generateAdaptiveHdpiIcons(sourceBuffer) {
  console.log('\n🎨 Generating adaptive hdpi icons (162x162)...');

  for (const [density, config] of Object.entries(ADAPTIVE_HDPI)) {
    const dirPath = path.join(OUTPUT_DIR, density);
...
async function generateHdpiXMLFiles() {
  console.log('\n📄 Generating hdpi XML configuration files...');
...
<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
  ...
  const dirPath = path.join(OUTPUT_DIR, 'mipmap-hdpi');
...
# ============================================================================
# ImageMagick Commands for hdpi Mipmap Icons
# ============================================================================

# 1. Create mipmap-hdpi regular icon (72x72)
convert source.png -resize 72x72 -background none -gravity center \\
  -extent 72x72 -antialias android/app/src/main/res/mipmap-hdpi/ic_launcher.png

# 2. Create mipmap-hdpi round icon (72x72)
convert source.png -resize 72x72 -background none -gravity center \\
  -extent 72x72 -format png - | \\
  convert - -draw "circle 36,36 36,0" -alpha set -compose copy-opacity -composite \\
  -antialias android/app/src/main/res/mipmap-hdpi/ic_launcher_round.png

# 3. Adaptive hdpi icons (162x162)
# Foreground
convert source.png -resize 98x98 -background none -gravity center \\
  -extent 162x162 -antialias android/app/src/main/res/mipmap-hdpi/ic_launcher_foreground.png

# Background
convert -size 162x162 xc:#1976d2 android/app/src/main/res/mipmap-hdpi/ic_launcher_background.png

# Round foreground
convert source.png -resize 98x98 -background none -gravity center \\
  -extent 162x162 -format png - | \\
  convert - -draw "circle 81,81 81,0" -alpha set -compose copy-opacity -composite \\
  android/app/src/main/res/mipmap-hdpi/ic_launcher_round_foreground.png

# Round background
convert -size 162x162 xc:#1976d2 -format png - | \\
  convert - -draw "circle 81,81 81,0" -alpha set -compose copy-opacity -composite \\
  android/app/src/main/res/mipmap-hdpi/ic_launcher_round_background.png
`;
...
// Generate all hdpi mipmap icons
generateHdpiMipmapIcons().catch(console.error);