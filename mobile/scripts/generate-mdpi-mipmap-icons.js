...
// ============================================================================
// Android mdpi Mipmap Icon Generator
// ============================================================================

/**
 * Script to generate all Android mdpi mipmap app icons
 * 
 * Run: node scripts/generate-mdpi-mipmap-icons.js
 * 
 * Prerequisites:
...
// mdpi mipmap icon configurations (48x48)
const MDPI_MIPMAP_CONFIG = {
  'mipmap-mdpi': {
    regular: { size: 48, name: 'ic_launcher.png' },
    round: { size: 48, name: 'ic_launcher_round.png' },
  },
};

// Adaptive mdpi icons (Android 8.0+)
const ADAPTIVE_MDPI = {
  'mipmap-mdpi': {
    size: 108,
...
async function generateMdpiMipmapIcons() {
  console.log('🎨 Generating Android mdpi mipmap app icons...');
  console.log('================================================\n');
  ...
  // Generate mdpi mipmap icons
  await generateStandardMdpiMipmapIcons(sourceBuffer);
  await generateAdaptiveMdpiIcons(sourceBuffer);
  await generateMdpiXMLFiles();

  console.log('\n🎉 All Android mdpi mipmap icons generated successfully!');
  console.log('================================================');
}

// ============================================================================
// Generate Standard mdpi Mipmap Icons
// ============================================================================

async function generateStandardMdpiMipmapIcons(sourceBuffer) {
  console.log('📱 Generating standard mdpi mipmap icons (48x48)...');

  for (const [density, config] of Object.entries(MDPI_MIPMAP_CONFIG)) {
...
// ============================================================================
// Generate Adaptive mdpi Icons
// ============================================================================

async function generateAdaptiveMdpiIcons(sourceBuffer) {
  console.log('\n🎨 Generating adaptive mdpi icons (108x108)...');

  for (const [density, config] of Object.entries(ADAPTIVE_MDPI)) {
    const dirPath = path.join(OUTPUT_DIR, density);
...
async function generateMdpiXMLFiles() {
  console.log('\n📄 Generating mdpi XML configuration files...');
...
<adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
  ...
  console.log(`   ✅ ic_launcher.xml → mipmap-mdpi`);
...
# ============================================================================
# ImageMagick Commands for mdpi Mipmap Icons
# ============================================================================

# 1. Create mipmap-mdpi regular icon (48x48)
convert source.png -resize 48x48 -background none -gravity center \\
  -extent 48x48 -antialias android/app/src/main/res/mipmap-mdpi/ic_launcher.png
...
convert source.png -resize 48x48 -background none -gravity center \\
  ...
  -antialias android/app/src/main/res/mipmap-mdpi/ic_launcher_round.png

# 3. Adaptive mdpi icons (108x108)
# Foreground
convert source.png -resize 65x65 -background none -gravity center \\
  -extent 108x108 -antialias android/app/src/main/res/mipmap-mdpi/ic_launcher_foreground.png

# Background
convert -size 108x108 xc:#1976d2 android/app/src/main/res/mipmap-mdpi/ic_launcher_background.png
...
// Generate all mdpi mipmap icons
generateMdpiMipmapIcons().catch(console.error);